...
export const ACTION_LINKS = {
  analysis: [
    { id: 'whois', label: 'Whois Lookup', icon: ICO.globe, url: d => `https://whois.domaintools.com/${d}` },
    { id: 'google', label: 'Google Search', icon: ICO.search, url: d => `https://www.google.com/search?q=${d}` },
    { id: 'spyfu', label: 'SpyFu SEO', icon: ICO.chart, url: d => `https://www.spyfu.com/overview/domain?query=${d}` },
    { id: 'dotdb', label: 'DotDB Popularity', icon: ICO.barChart, url: d => `https://dotdb.com/search?keyword=${d}` },
    { id: 'archive', label: 'Wayback Machine', icon: ICO.clock, url: d => `https://web.archive.org/web/*/${d}` },
    { id: 'namebio', label: 'Comparable Sales', icon: ICO.dollar, url: d => `https://namebio.com/?term=${d.split('.')[0]}` },
  ],
  seo: [
    { id: 'spamcheck', label: 'Spam Check', icon: ICO.alert, url: d => `https://www.spamhaus.org/query/domain/${d}` },
    { id: 'trademark', label: 'Trademark Check', icon: ICO.shield, url: () => 'https://tmsearch.uspto.gov/search/search-information' },
  ],
  appraisal: [
    { id: 'godaddy', label: 'GoDaddy Appraisal', icon: ICO.dollar, url: d => `https://www.godaddy.com/domain-value-appraisal/appraisal/?domainToCheck=${d}` },
    { id: 'dynadot-app', label: 'Dynadot Appraisal', icon: ICO.barChart, url: d => `https://www.dynadot.com/domain/appraisal?domain=${d}` },
  ],
  registrar: [
    { id: 'namecheap', label: 'Namecheap', icon: ICO.cart, url: d => `https://www.namecheap.com/domains/registration/results/?domain=${d}` },
    { id: 'dynadot', label: 'Dynadot', icon: ICO.cart, url: d => `https://www.dynadot.com/domain/search.html?domain=${d}` },
    { id: 'namesilo', label: 'NameSilo', icon: ICO.cart, url: d => `https://www.namesilo.com/domain/search-domains?query=${d}` },
    { id: 'spaceship', label: 'Spaceship', icon: ICO.cart, url: d => `https://spaceship.com/domain/search?query=${d}` },
    { id: 'porkbun', label: 'Porkbun', icon: ICO.cart, url: d => `https://porkbun.com/checkout/search?q=${d}` },
    { id: 'ud', label: 'Unstoppable', icon: ICO.globe, url: d => `https://unstoppabledomains.com/search?searchTerm=${d}` },
  ]
};
...
export function createActionMenu(card, domain) {
  ...
  ACTION_LINKS.analysis.forEach(a => {
    html += `<a href="${a.url(domainName)}" target="_blank" rel="noopener" class="action-link">${svgIcon(a.icon)}${a.label}</a>`;
  });
  html += '</div>';
  ...
  if (isAvailable) {
    ...
    ACTION_LINKS.registrar.forEach(a => {
      html += `<a href="${a.url(domainName)}" target="_blank" rel="noopener" class="action-link action-register">${svgIcon(a.icon)}${a.label}</a>`;
    });
    html += '</div>';
  }
  ...
  html += `<button class="action-btn action-send" data-action="send-to-analyzer">${svgIcon(ICO.chart)}Send to Smart Analyzer</button>`;
