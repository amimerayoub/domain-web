...
export function renderBulkResults(domains) {
  ...
  domains.forEach((d, i) => {
    ...
    const a = d.available;
    item.innerHTML = `<span class="bulk-domain">${d.name}</span><span class="bulk-status ${a ? 'bulk-available' : 'bulk-taken'}">${a ? '✅ Available' : '❌ Taken'}</span>`;
  ...
  domains.forEach((d, i) => {
    ...
    html += `<div class="domain-card-row" data-idx="${i}" style="animation-delay:${i * 0.03}s">
      ...
      <div class="dc-identity">
        ...
        <div class="dc-meta">
          <span class="dc-status ${d.available ? 'dc-avail' : 'dc-taken'}">
            <span class="dc-status-dot"></span>${d.available ? 'Available' : 'Registered'}
          </span>
...
function renderAnalyzerCards(container, domains) {
  ...
  domains.forEach((d, i) => {
    ...
    const st = d.available ? 'Available' : 'Taken';
    const favActive = isFavorite(d.name);
    ...
    card.innerHTML = `
      ...
      <div class="domain-status" style="font-size:.75rem;color:var(--text-secondary)">${st}</div>
      <div class="card-actions">
        <button class="btn-copy" data-domain="${d.name}">
          <svg viewBox="0 0 24 24" fill="none"><rect x="9" y="9" width="13" height="13" rx="2" stroke="currentColor" stroke-width="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1" stroke="currentColor" stroke-width="2"/></svg>
          Copy
        </button>
      </div>`;
    ...
    // Copy handler
    const copyBtn = card.querySelector('.btn-copy');
    if (copyBtn) copyBtn.addEventListener('click', function () { copyText(d.name, this); });
    ...
    if (nameEl) nameEl.addEventListener('click', () => openDomainDetails(d));
...
function openDomainDetails(d) {
  ...
  EXTENSIONS.forEach(ext => {
    ...
    const avail = isCurrent ? d.available : Math.random() > 0.4;
    extHtml += `<div class="ext-item ${isCurrent ? 'current-ext' : ''}">
      <span class="ext-name">${ext}</span>
      <span class="ext-status ${avail ? 'ext-available' : 'ext-registered'}">${avail ? 'Available' : 'Registered'}</span>
    </div>`;
  });
