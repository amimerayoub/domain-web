...
// ============================================================
// BASIC MODE: simulate availability check (random for demo)
// ============================================================

function checkAvailabilityBasic(domain) {
  // In a real app, this would call an API
  // For demo: simulate availability
  const tld = domain.split('.').pop();
  const name = domain.split('.')[0];
  const score = name.length * 7 + (tld === 'com' ? 30 : 10);
  return { available: rand(1, 100) > score * 0.3 };
}
    ...
    if (basicDomains && basicDomains.length) {
      return {
        ...
        domains: basicDomains.map(d => ({
          ...
          available: checkAvailabilityBasic(d).available
        })),
        parseStats
      };
    }
  ...
  return {
    ...
    domains: parsed.map(item => {
      ...
      return {
        ...
        available: item.available !== null ? item.available : rand(1, 100) > 35,
        metrics: {
